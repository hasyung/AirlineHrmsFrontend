        ...
        link: {
            post: function postLink(scope) {
                ...
                function nestedStates(steps) {
                    ...
                    if(steps.length > 0 && lastStep && startsWith(_.last(steps).name, lastStep.name)){
                        temp = steps.reduce(function(res, value){

                            if(startsWith(value.name, lastStep.name) && value.name !== lastStep.name ) {
                                res.push(value)
                            }
                            return res
                        }, [])
                ...
                var renderBreadcrumb = function() {
                    ...
                    if(!nested){
                    ...
                    else if( steps.length && scope.steps.length && _.last(steps).name == _.last(scope.steps).name  ){

                        scope.steps = scope.steps.splice(scope.steps.length - 1 , 1, nested[0])
                        // deregisterWatchers(labelWatchers);
                        angular.forEach(nested, function (step) {
                            if (step.ncyBreadcrumb && step.ncyBreadcrumb.label) {
                                var parseLabel = $interpolate(step.ncyBreadcrumb.label);
                                step.ncyBreadcrumbLabel = parseLabel(viewScope);
                                // Watcher for further viewScope updates
                        ...
                        scope.steps = scope.steps.concat(nested)
                    }



                    // lastStep = steps[steps.length - 1]
                    // lastStep.includes( _.last($scope.steps) )
